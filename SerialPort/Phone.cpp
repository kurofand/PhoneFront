...
//TODO: error handling for failing parse signal value
		...
		case ATResponse::COPS:
		{
			...
			auto *root=engine_->rootObjects().first();
			if(root)
				QMetaObject::invokeMethod(root, "updateOperatorNameVal", Q_ARG(QVariant, QString::fromStdString(operatorName_)));
			break;
		}
...
//TODO: handle cases when size differs from 3
			if(responseStr.size()==3)
			{
				...
				auto *root=engine_->rootObjects().first();
				if(root)
					QMetaObject::invokeMethod(root, "updateConnectionStatusVal", Q_ARG(QVariant, QString::fromUtf8(connectionStatus.at(status_))));
			}
            else
